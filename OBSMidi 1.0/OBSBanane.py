...
#These variables are here for Memory:
LastScene = 99
LastTransition = 99
...
def SetTransition(BUTTONCONF):
	...
	if "Transition" in BUTTONCONF:
		...
		try:
			#Here we turn the last led off
			try:
				midi_out.write_short(0x90, LastTransition, 0)
			except:
				pass
			#Here we turn the next led on
			if "Led" in BUTTONCONF:
				midi_out.write_short(0x90, BUTTONCONF["Led"], 1)
				LastTransition = BUTTONCONF["Led"]
			else:
